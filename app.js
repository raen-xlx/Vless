...
console.log("Nodejs自动化无交互Trojan代理脚本");
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
...
async function main() {
    // Custom formatted password/UUID structure
    const PASSWORD = getVariableValue('UUID', '01110010011000010110010101101110');
    console.log('你的Trojan密码:', PASSWORD);
    ...
    const httpServer = http.createServer((req, res) => {
        ...
        if (req.url === `/${PASSWORD}`) {
            ...
            const singleNode = `trojan://${PASSWORD}@${CUSTOM_ADDRESS}:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}`;
            
            const multiNodes = `trojan://${PASSWORD}@${CUSTOM_ADDRESS}:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.16.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.17.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.18.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.19.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.20.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.21.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}
trojan://${PASSWORD}@104.22.0.0:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${currentCloudrunHost}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}`;
...
<head>
    ...
    <title>RAEN PRO TROJAN DASHBOARD</title>
    <style>
        :root {
            --bg-main: #080c14;
...
<div class="container">
    ...
    <div class="info-grid">
        ...
        <div class="info-card uuid">
            ...
            <div class="card-content">
                <div class="label">Trojan Password</div>
...
<script>
    ...
    wss.on('connection', ws => {
        ws.once('message', msg => {
            // Trojan protocol parse over WS: 
            // Structure: [Hex Password Hash (56 bytes)] + [CRLF (2 bytes)] + [COMMAND (1 byte)] + ...
            // For a lightweight customized inline setup, we can use the original credential array validation bounds:
            const passBuffer = Buffer.from(PASSWORD, 'utf8');
            
            // Validate protocol request packet lengths & structure basic safety
            if (msg.length < passBuffer.length) return;
            
            // Check credentials matched safely
            for (let i = 0; i < passBuffer.length; i++) {
                if (msg[i] !== passBuffer[i]) return;
            }
            
            // Header parsing parameters positioning relative to initial buffer offset
            let offset = passBuffer.length + 2; // skip password and CRLF (\r\n)
            const COMMAND = msg[offset]; 
            offset += 1; // move past command byte
            
            const ATYP = msg[offset];
            offset += 1; // move past ATYP byte
            ...
            if (ATYP === 1) { // IPv4
                ...
                offset += 1;
                host = new TextDecoder().decode(msg.slice(offset, offset + domainLen));
                offset += domainLen;
            } else if (ATYP === 3) { // IPv6
            ...
            const port = msg.slice(offset, offset + 2).readUInt16BE(0);
            offset += 2; // skip port
            
            // Skip payload CRLF inside Trojan standard validation if present
            const payloadStart = offset;

            const duplex = createWebSocketStream(ws);
            net.connect({ host, port }, function () {
                this.write(msg.slice(payloadStart));
                duplex.on('error', () => { }).pipe(this).on('error', () => { }).pipe(duplex);
            }).on('error', () => { });
        }).on('error', () => { });
    });
    ...
    const logDomain = CONFIG_DOMAIN || 'vless-930976547673.europe-west1.run.app';
    console.log(`trojan-ws-tls节点分享 (基本配置): trojan://${PASSWORD}@${CUSTOM_ADDRESS}:443?security=tls&sni=${CUSTOM_SNI}&fp=chrome&type=ws&host=${logDomain}&path=${URL_PATH}#Tj-ws-tls-pro-${NAME}`);
}
main();
